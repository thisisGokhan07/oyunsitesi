...
async function run() {
  console.log('🎮 GameMonetize oyunları REST API ile ekleniyor...\n');

  // 1. İlk kategoriyi al
  const catResponse = await fetch(`${url}/rest/v1/categories?published=eq.true&select=id&limit=1`, {
    headers: {
      'apikey': serviceKey,
  ...
  if (!catResponse.ok) {
    ...
    console.error('❌ Kategori hatası:', catResponse.status, errorText);
    return;
  }
  ...
  for (const game of games) {
    try {
      ...
      const checkResponse = await fetch(`${url}/rest/v1/content?slug=eq.${game.slug}&select=id`, {
        headers: {
          'apikey': serviceKey,
          'Authorization': `Bearer ${serviceKey}`,
        },
      ...
      // Oyunu ekle
      const insertResponse = await fetch(`${url}/rest/v1/content`, {
        method: 'POST',
        headers: {
          'apikey': serviceKey,
          'Authorization': `Bearer ${serviceKey}`,
      ...
      if (!insertResponse.ok) {
        ...
        console.error(`❌ Hata (${game.title}):`, insertResponse.status, errorText);
        errorCount++;
      } else {
